...
def question_1(df_balances):
    ...
    """
    # #A type 1 default occurs on a loan when any scheduled monthly repayment is not met in full.
    # #First we calculate the number of defaulted loans. 
    # #Check where the actual repayment is less than the scheduled repayment.
    # #Get the unique loanID's of the defaulted loans.
    # defaulted_loans = df_balances[df_balances['ActualRepayment'] < df_balances['ScheduledRepayment']]['LoanID'].unique()

    # #Get the total number of loans.
    # total_loans = df_balances['LoanID'].unique().shape[0]

    # #Next we calculate the percentage of defaulted loans.
    # default_rate_percent = (len(defaulted_loans) / total_loans) * 100
    fin = 0.0
